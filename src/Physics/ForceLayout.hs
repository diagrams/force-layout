...
initParticle p = Particle p zero
...
type Forces v n = [(U.Vector (Int,Int), Point v n -> Point v n -> v n)]
...
ensembleStep
  ...
  -> Ensemble v n
  -> Ensemble v n
ensembleStep d (Ensemble fs ps) = Ensemble fs ps'
  where
    ps' = V.imap updateParticle ps
    updateParticle i p = Particle ((p^.pos) .+^ v') v'
      where v' = d *^ (p^.vel ^+^ (fv V.! i))
    n = V.length ps
    -- the forces acting of each particle
    fv = V.create $ do
      fmv <- M.replicate n zero
      F.for_ fs $ \(edges,f) ->
        U.forM_ edges $ \(i1,i2) -> do
          let !force = f (ps^?!ix i1.pos) (ps^?!ix i2.pos)
          M.modify fmv (^+^ force) i1
          M.modify fmv (^-^ force) i2
      return fmv
{-# INLINE ensembleStep #-}

-- | Recalculate all the forces acting in the next time step of an
--   ensemble.
-- calcForces :: (Additive v, Num n) => Ensemble v n -> Vector (v n)
-- calcForces (Ensemble fs ps) = create $ do
...
data ForceLayoutOpts n = FLOpts
  { _damping     :: n
    -- ^ Damping factor to be applied at each step. Should be between 0
    --   and 1. The default is 0.8.
  , _energyLimit :: Maybe n
    -- ^ Kinetic energy below which simulation should stop. If @Nothing@,
    --   pay no attention to kinetic energy. The default is @Just 0.001@.
  , _stepLimit   :: Maybe Int
    -- ^ Maximum number of simulation steps.  If @Nothing@, pay no
    --   attention to the number of steps.  The default is @Just 1000@.
  }
